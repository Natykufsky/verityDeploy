window.verityServerMetrics = function verityServerMetrics(options) {
    return {
        ...
        metricEntries(metricValue) {
            if (Array.isArray(metricValue)) {
                return metricValue.map((value, index) => [index, value]);
            }
        ...
        disconnectBridge() {
            ...
            if (this.bridgeSocket) {
                try {
                    this.bridgeSocket.close(1000, 'metrics closed');
                } catch (error) {}
            }
