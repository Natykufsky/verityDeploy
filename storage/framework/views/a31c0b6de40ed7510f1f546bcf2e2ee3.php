...
<details open class="deployment-frost-card rounded-3xl p-5">
    <summary class="flex cursor-pointer list-none flex-wrap items-start justify-between gap-4">
        <div class="space-y-3">
            ...
            <div class="space-y-2">
                ...
                <p class="max-w-3xl text-sm leading-7 text-slate-300 md:text-base">
                    <?php echo e($record->shared_env_summary); ?>
        ...
        <div class="flex flex-wrap items-center gap-2">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $badges; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $badge): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <span class="inline-flex items-center rounded-full border border-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] <?php echo e(match ($badge['color']) {
    ...
    <div class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div class="deployment-frost-panel rounded-2xl p-4">
            ...
            <div class="mt-2 text-sm font-semibold text-white"><?php echo e($record->server?->name ?? 'No server assigned'); ?></div>
            <p class="mt-2 text-sm leading-6 text-slate-400">
                <?php echo e($record->server?->connection_type ? ucfirst((string) $record->server->connection_type) . ' connection' : 'No connection type set yet.'); ?>
        ...
        <div class="deployment-frost-panel rounded-2xl p-4">
            ...
            <div class="mt-2 text-sm font-semibold text-white"><?php echo e($record->primary_domain ?? 'No primary domain yet'); ?></div>
            <p class="mt-2 text-sm leading-6 text-slate-400">
                <?php echo e($record->domain_summary); ?>

            </p>
            <div class="mt-3 space-y-2 text-sm leading-6 text-slate-300">
                <div><span class="font-semibold text-white">SSL:</span> <?php echo e($record->ssl_summary); ?></div>
                <div><span class="font-semibold text-white">HTTPS redirect:</span> <?php echo e($record->force_https_summary); ?></div>
                <div><span class="font-semibold text-white">DNS:</span> <?php echo e($record->dns_provider_summary); ?></div>
            </div>
        </div>
        ...
        <div class="deployment-frost-panel rounded-2xl p-4">
            ...
            <p class="mt-2 text-sm leading-6 text-slate-400">
                <?php echo e($record->current_release_status === 'active' ? 'This site is ready for the next deployment cycle.' : 'Start with the setup step that removes the current blocker.'); ?>
        ...
        <div class="deployment-frost-panel rounded-2xl p-4">
            ...
            <div class="mt-2 text-sm font-semibold text-white"><?php echo e($record->current_release_path ?? 'Not active yet'); ?></div>
            <p class="mt-2 text-sm leading-6 text-slate-400">
                <?php echo e($record->last_successful_deploy_badge); ?>
    ...
    <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <div class="deployment-frost-panel rounded-2xl p-4">
            ...
            <div class="mt-2 text-2xl font-semibold text-white"><?php echo e($record->deployments()->count()); ?></div>
        </div>
        <div class="deployment-frost-panel rounded-2xl p-4">
            <div class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">Webhooks</div>
            <div class="mt-2 text-sm font-semibold text-white"><?php echo e($record->github_webhook_health); ?></div>
            <p class="mt-1 text-sm text-slate-400"><?php echo e($record->github_webhook_sync_health); ?></p>
        </div>
        <div class="deployment-frost-panel rounded-2xl p-4">
            <div class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">Runtime</div>
            <div class="mt-2 text-sm font-semibold text-white"><?php echo e($record->shared_env_mode === 'custom' ? 'Custom override' : 'Generated env'); ?></div>
            <p class="mt-1 text-sm text-slate-400"><?php echo e($record->php_version ?? 'PHP version not set'); ?></p>
        </div>
    </div>
</details>
<?php /**PATH C:\Users\Natykufsky\Desktop\Apps\php\verityDeploy\resources\views/filament/sites/site-overview.blade.php ENDPATH**/ ?>